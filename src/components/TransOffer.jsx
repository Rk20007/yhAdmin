...
import { TransAddOfferAPI, TransGetOfferAPI } from "../api/transport.api";
...
const TransOffer = () => {
  ...
  const getOffers = async () => {
    const getData = await TransGetOfferAPI();
  ...
  const handleSubmit = async () => {
    const res = await TransAddOfferAPI(payload);
    if (res.status) {
  ...
  return (
    <main className="flight-offer">
      ...
      >
        <TransOfferTable offers={offers} getOffers={getOffers} />
      </section>
    </main>
  );
};

...
import { OfferDeleteAPI } from "../api/offer.api";
import OfferFlightModal from "./OfferFlightModal";
import OfferHotelModal from "./OfferHotelModal";
...
export default function OfferHotelTable({ offers, getOffers }) {
  const [open, setOpen] = React.useState(false);
  const [updateData, setUpdateData] = React.useState({});

  const hanldeDelete = async (id) => {
    const response = await OfferDeleteAPI({ _id: id });
    if (response.status) {
      getOffers();

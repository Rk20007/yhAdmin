...
import { TransDeleteOfferAPI } from "../api/transport.api";
...
export default function TransOfferTable({ offers, getOffers }) {
  const [open, setOpen] = React.useState(false);
  const [updateData, setUpdateData] = React.useState({});

  const hanldeDelete = async (id) => {
    const response = await TransDeleteOfferAPI({ _id: id });
    if (response.status) {
      getOffers();

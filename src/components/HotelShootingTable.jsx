...
import { HotelDeleteShootingAPI } from "../api/hotel.api";
import HotelDealModal from "./HotelDealModal";
import HotelShootingModal from "./HotelShootingModal";
...
export default function HotelShootingTable({ deals, getDeals }) {
  const [open, setOpen] = React.useState(false);
  const [updateData, setUpdateData] = React.useState({});

  const hanldeDelete = async (id) => {
    const response = await HotelDeleteShootingAPI({ _id: id });
    if (response.status) {
      getDeals();
        ...
        <TableBody>
          {deals?.map((ele) => (
            <StyledTableRow key={ele._id}>
              ...
              <StyledTableCell align="center">{ele.stars}</StyledTableCell>
              <StyledTableCell align="center">
                {ele.reviews_count}
              </StyledTableCell>
